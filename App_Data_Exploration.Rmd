...
#contab_df$can_par_aff[which(contab_df$can_par_aff == "NA")] 

c_exp_sum <- contab_df %>% 
  group_by(can_off,can_off_sta,can_par_aff,can_nam, spe_nam, sup_opp) %>% 
  summarise(sum_exp = sum(exp_amo))
...
sen_top_donor <- senate_df %>% 
  group_by(spe_nam, can_off_sta, can_par_aff, sup_opp) %>% 
  summarise(sum_exp = sum(exp_amo)) %>% 
  arrange(desc(sum_exp))
